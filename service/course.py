...
from service.auth import Authorization  # Assuming Authorization class is imported from auth.py
...
def get_uncompleted_course_components(me: Authorization, ignore_course_ids: Optional[List[int]] = None) -> List[Any]:
    ...
    try:
        for comp in components:
            attendance = attendanceItems(comp['course_id'], comp['item_id'], me.token).result
            if attendance.unlock_at is not None and attendance.due_at is not None:
                if datetime.fromisoformat(attendance.unlock_at.replace("Z", "")).replace(tzinfo=timezone.utc) < now < datetime.fromisoformat(attendance.due_at.replace("Z", "")).replace(tzinfo=timezone.utc):
